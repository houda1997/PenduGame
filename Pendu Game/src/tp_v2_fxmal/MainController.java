...
public class MainController implements Initializable {
    ...
    private Session session ; //la session courante 
  ...
  //evenement : cliquer sur le button Commencer a jouer
   ...
   //-------------------- Afficher l'image ------------------------//
    ...
    Font myFont = new Font ("Times New Roman", 80);
    this.etape.setText(Integer.toString(i) + "/" + Integer.toString(6));
    }
    public void FinJeu (String text) {
        
            final Button sortir = new Button ("Sortir");
            final Button save = new Button ("Enregistrer le score");
            final Button refaire = new Button ("Nouvelle session");
            final Text sauvText = new Text ();
            sauvText.setText("Vous avez sauvgarder le score avec succes");
            sauvText.setVisible(false);
            Font myFont = new Font("Times New Roman",12);
            sauvText.setFont(myFont);
            sauvText.setFill(Color.GREEN);
            
            
            sortir.setOnAction(new EventHandler<ActionEvent>() { 
                   @Override              
                   public void handle(ActionEvent e) {
                       Platform.exit();
                       System.exit(0);
            ...
            save.setOnAction(new EventHandler<ActionEvent>() { 
                   @Override              
                   public void handle(ActionEvent e) {
                       if (session.getJoueur().getSessionScore() > session.getJoueur().getMeilleurScore()) {
                           session.getJoueur().setMeilleurScore(session.getJoueur().getSessionScore());
                           session.getJoueur().Sauvgarder();
                           
                   }
                       sauvText.setVisible(true);
                   }     
               });
            refaire.setOnAction(new EventHandler<ActionEvent>()  { 
                   ...
                   public void handle(ActionEvent e)  {
                      ...
                      FXMLLoader fxmlLoader = new FXMLLoader(getClass().getResource("Main.fxml"));
                       ...
                       Joueur joueur= session.getJoueur();
                       joueur.setSessionScore(0);
                       session = new Session(joueur);
                       controller.setSession(session);
                        Scene scene = new Scene(parent);
                        Stage appStage = (Stage)start.getScene().getWindow();
            ...
            buttons.getChildren().addAll(save, refaire, sortir);
             VBox dialogVbox = new VBox(60);
             Text txt = new Text (text);
             myFont = new Font("Times New Roman",25);
             txt.setFont(myFont);
             
            
             dialogVbox.alignmentProperty().set(Pos.TOP_CENTER);
             dialogVbox.getChildren().add(txt);
             dialogVbox.alignmentProperty().set(Pos.TOP_CENTER);
             dialogVbox.getChildren().add(buttons);
             dialogVbox.alignmentProperty().set(Pos.TOP_CENTER);
             dialogVbox.getChildren().add(sauvText);
             
             Scene dialogScene = new Scene(dialogVbox, 400, 200);
             dialogScene.setFill(Color.FLORALWHITE);
             dialog.setScene(dialogScene);
             dialog.show();
    }
    ...
    public void ClickCaseZeroChance (TextField txt){
          ...
          if (text.length() == 1 || (text.length() == 2 && text.toCharArray()[1] == ' '))  //verifier si le text introduit est un chracter 
          ...
          if (tabMots[session.getnumMotCourant()].ScoreLettre(l, j)) { // le score est incrementer 
              ...
              if (session.getMotRate() <6){
                if (session.getnumMotCourant() + 1 >= session.getnbrMot())  
                    FinJeu (" BRAVO! ");
                else {
